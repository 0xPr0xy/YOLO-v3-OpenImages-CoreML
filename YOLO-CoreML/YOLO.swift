...
class YOLO {
    ...
    private let blockSize: Float = 32
    private let gridHeight = 19
    private let gridWidth = 19
    private let boxesPerCell = 5
    private let numClasses = 371 // should be 601

    let model = yolo_openimages_1()
    ...
    public func computeBoundingBoxes(features: MLMultiArray) -> [Prediction] {
        assert(features.count == 1818*19*19)
        ...
        for cy in 0..<gridHeight {
            for cx in 0..<gridWidth {
                for b in 0..<boxesPerCell {
                    ...
                    // cause of crash is due to anchors not having 9 values but 8
                    // but has 8 anchors in yolov3-openimages.cfg..
